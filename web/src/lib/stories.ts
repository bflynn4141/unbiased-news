...
export const mockStories: Story[] = [
  {
    ...
    sources: [
      { name: 'Fox News', lean: 'right', biasScore: 78, url: '#', headline: 'Freedom Caucus to Trump: DHS funding must stay', snippet: 'Conservative members demand full funding for immigration enforcement agencies, citing border security as top priority. Representatives argue that any cuts to ICE or CBP would endanger American communities and undermine the rule of law.', publishedAt: new Date().toISOString() },
      { name: 'NBC News', lean: 'left-center', biasScore: 32, url: '#', headline: 'Democrats threaten to block DHS funding after another killed', snippet: 'Senate Democrats vow opposition to ICE tactics following reports of aggressive enforcement actions in communities. Civil rights groups call for oversight and accountability measures before any funding approval.', publishedAt: new Date().toISOString() },
      { name: 'CNBC', lean: 'center', biasScore: 51, url: '#', headline: 'Government may shut down early Saturday', snippet: 'Tax season delays, federal workers furloughed if no deal reached by Friday midnight. Economic analysts warn of GDP impact and disrupted government services affecting millions of Americans.', publishedAt: new Date().toISOString() },
      { name: 'PBS', lean: 'center', biasScore: 48, url: '#', headline: 'Democrats vow to block Homeland Security funding', snippet: 'Bipartisan tensions rise as deadline approaches. Both parties accuse the other of putting politics above national interests as federal employees brace for potential furloughs.', publishedAt: new Date().toISOString() },
      { name: 'Reuters', lean: 'center', biasScore: 50, url: '#', headline: 'U.S. government shutdown looms as parties clash over immigration', snippet: 'With hours until deadline, negotiations remain stalled on Department of Homeland Security funding. Congressional leaders scheduled emergency meeting as markets react to uncertainty.', publishedAt: new Date().toISOString() },
      { name: 'The Hill', lean: 'center', biasScore: 52, url: '#', headline: 'Shutdown odds rise as DHS funding fight intensifies', snippet: 'Congressional insiders give shutdown 70% probability without last-minute deal. Both parties prepare blame messaging while moderate lawmakers seek compromise solution.', publishedAt: new Date().toISOString() },
    ],
    analysis: {
  ...
  },
  {
    ...
    sources: [
      { name: 'CNN', lean: 'left-center', biasScore: 35, url: '#', headline: 'Trump signs flurry of executive orders', snippet: 'Critics call moves unconstitutional as new president signs record number of day-one actions. Legal experts warn several orders will face immediate court challenges.', publishedAt: new Date().toISOString() },
      { name: 'Wall Street Journal', lean: 'right-center', biasScore: 62, url: '#', headline: 'Trump moves quickly on policy priorities', snippet: 'Markets react positively to deregulation signals. Business leaders praise decisive action on energy permits and federal hiring freeze.', publishedAt: new Date().toISOString() },
      { name: 'AP News', lean: 'center', biasScore: 50, url: '#', headline: 'Trump signs executive orders on first day', snippet: 'Orders cover immigration, energy, federal hiring freeze, and withdrawal from international agreements. Total count exceeds any previous inauguration day.', publishedAt: new Date().toISOString() },
      { name: 'New York Times', lean: 'left-center', biasScore: 33, url: '#', headline: 'Trump wastes no time reshaping government', snippet: 'Sweeping orders target immigration policy, environmental regulations, and diversity programs. Civil rights organizations announce legal challenges.', publishedAt: new Date().toISOString() },
    ],
    analysis: {
  ...
  },
  {
    ...
    sources: [
      { name: 'Bloomberg', lean: 'center', biasScore: 52, url: '#', headline: 'Fed expected to hold rates in January', snippet: 'Inflation data supports pause in cuts as Fed officials signal patience. Markets pricing in no change at upcoming FOMC meeting.', publishedAt: new Date().toISOString() },
      { name: 'Reuters', lean: 'center', biasScore: 50, url: '#', headline: 'Federal Reserve rate decision looms', snippet: 'Economic indicators point to steady policy as central bank balances growth and inflation concerns. Employment remains strong while price pressures ease.', publishedAt: new Date().toISOString() },
      { name: 'Financial Times', lean: 'center', biasScore: 53, url: '#', headline: 'Fed to stand pat amid economic uncertainty', snippet: 'Central bankers eye tariff implications and fiscal policy changes. Minutes from December meeting revealed divided opinions on rate path.', publishedAt: new Date().toISOString() },
    ],
    analysis: {
